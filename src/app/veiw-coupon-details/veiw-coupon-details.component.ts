...
@Component({
  selector: 'app-veiw-coupon-details',
  templateUrl: './veiw-coupon-details.component.html',
  styleUrls: ['./veiw-coupon-details.component.css']
})
export class VeiwCouponDetailsComponent implements OnInit {
  sub: any;
  id: any;
  ...
  constructor(private route: ActivatedRoute, private formBuilder: FormBuilder, private router: Router, private service: SharedServiceService) {
    ...
    this.getSelectedCouponDetails(this.id.coupon_id);
  ...
  getSelectedCouponDetails(id) {
    console.log(id);
    this.service.getCouponByID(id).subscribe(res => {
      console.log(res);
      this.getDetails = res;
      if (this.getDetails.store_id.length > 1) {
        this.selectedStore = "for_admin";
      } else {
        this.selectedStore = this.getDetails.store_id[0];
      }
      console.log(this.selectedStore);
      if (this.getDetails.is_amount === true) {
        this.addNewCouponForm.get('value_type').setValue("amount");
        this.addNewCouponForm.get('coupon_amount').setValue(this.getDetails.coupon_amount);
        this.startDate = this.getDetails.coupon_start_date.split('T');
        this.endDate = this.getDetails.coupon_end_date.split('T');
        console.log(this.startDate[0]);
        console.log(this.endDate[0]);
        $('#showThepersentage').css("display", "none");
        console.log("1");
      } else {
        this.addNewCouponForm.get('value_type').setValue("percentage");
        this.addNewCouponForm.get('coupon_amount').setValue(this.getDetails.coupon_percentage + '%');
  ...
  updateTheCoupons() {
    ...
    if (Number(this.a.minimum_cart_value.value) > Number(this.a.max_limit.value)) {
      if (this.a.value_type.value === "percentage" || this.shareThisInfo === "percentage") {
        console.log(this.a.coupon_amount.value.slice(0, -1));
        if (Number(this.a.coupon_amount.value.slice(0, -1)) <= 100) {
          if (this.a.store_id.value === "for_admin") {
            ...
            this.requestBody = {
              ...
              "is_percentage": true,
              "is_amount": false,
              "coupon_percentage": this.a.coupon_amount.value.slice(0, -1),
              "coupon_amount": null,
          ...
          this.service.updateTheCoupons(this.id.coupon_id, this.requestBody).subscribe(updateRes => {
            console.log(updateRes);
            this.updateResDetails = updateRes;
            $.notify({
              icon: "add_alert",
              message: this.updateResDetails.message
            }, {
              type: 'info',
              timer: 1000,
              placement: {
